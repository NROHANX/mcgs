...
CREATE OR REPLACE FUNCTION safe_truncate_table(target_table_name text)
RETURNS void AS $$
BEGIN
  ...
  IF EXISTS (
    ...
    EXECUTE format('TRUNCATE TABLE %I CASCADE', target_table_name);
    RAISE NOTICE 'Truncated table: %', target_table_name;
  ELSE
