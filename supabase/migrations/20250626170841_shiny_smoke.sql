/*
  # Complete MCGS Database Schema Reset
  
  1. New Tables
    - `users` - User profiles with role-based access
    - `service_categories` - Pre-populated service categories
    - `service_areas` - Pre-populated Nagpur service areas
    - `service_providers` - Provider business profiles
    - `provider_categories` - Provider-category relationships
    - `provider_service_areas` - Provider-area relationships
    - `services` - Individual services offered by providers
    - `service_bookings` - Main booking system
    - `booking_assignments` - Booking-provider assignments
    - `reviews` - Customer reviews and ratings
    - `contacts` - Contact form submissions
    - `support_tickets` - Customer support system
    - `admin_management` - Admin user management
    - `user_roles` - Extended role management

  2. Security
    - Enable RLS on all tables
    - Comprehensive access policies
    - Automatic triggers for timestamps
    - Rating system auto-updates
...
WHERE NOT EXISTS (
    ...
    WHERE service_areas.area_name = v.area_name 
    AND service_areas.city = v.city
);
