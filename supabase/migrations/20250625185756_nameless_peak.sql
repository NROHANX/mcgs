/*
  # Create Admin System with Approval Workflow

  1. New Tables
    - `admin_management` - Store admin users
    - `user_registration_requests` - Store pending registration requests
    - `user_approval_status` - Track user approval status
  ...
  3. Functions
    - Auto-create approval status when user signs up
...
-- If the admin user doesn't exist in auth.users yet, we'll handle it in the application