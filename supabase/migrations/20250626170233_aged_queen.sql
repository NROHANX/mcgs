/*
  # Initial MCGS Database Schema
  
  This migration creates the complete database schema for the MCGS (Max Care Grand Services) platform.
  ...
  5. provider_categories - Provider-category relationships
  6. provider_service_areas - Provider-area relationships
  7. services - Individual services offered
  8. service_bookings - Customer booking requests
  9. booking_assignments - Booking-provider assignments
  10. reviews - Customer reviews and ratings
  11. contacts - Contact form submissions
  12. support_tickets - Customer support system
  13. admin_management - Admin user management
  14. user_roles - Extended role management
...
-- Insert default service areas (Nagpur)
...
CREATE POLICY "Users can create their own provider profile" ON service_providers
...
CREATE POLICY "Providers can manage their own profile" ON service_providers
...
CREATE POLICY "Everyone can view approved providers" ON service_providers
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM users 
...
CREATE POLICY "Providers can manage their categories" ON provider_categories
    FOR ALL USING (
        EXISTS (
...
CREATE POLICY "Service areas are viewable by everyone" ON provider_service_areas
    FOR SELECT USING (true);
...
CREATE POLICY "Providers can view their assignments" ON booking_assignments
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM service_providers sp 
...
CREATE POLICY "Providers can update their assignment responses" ON booking_assignments
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM service_providers sp 
            WHERE sp.id = booking_assignments.provider_id 
...
CREATE POLICY "Admins can view admin management" ON admin_management
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY "Super admins can manage admin users" ON admin_management
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_management am 
            WHERE am.user_id = auth.uid() 
