/*
  ...
  1. New Tables
    ...
    - `admin_management` - Manages admin user permissions (renamed to avoid conflict)
  ...
  3. Functions
    - `approve_user_registration` - Handle user approval process
    - `reject_user_registration` - Handle user rejection process
...
-- Create admin_management table for admin management (renamed to avoid conflict with existing view)
